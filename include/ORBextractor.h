...
public:    
    ...
    int CheckMovingKeyPoints_segment(const cv::Mat &imGray, const cv::Mat &imS,std::vector<std::vector<cv::KeyPoint>>& mvKeysT,std::vector<cv::Point2f> T);
    int CheckMovingKeyPoints_detection(const cv::Mat &imGray, const std::vector<bbox_t> &detect_result, std::vector<std::vector<cv::KeyPoint>>& mvKeysT, std::vector<cv::Point2f> T);
    void DeleteOneRowOfMat(cv::Mat& object, int num);
    
    bool isInPerson(const cv::Point2f &coordinate, const std::vector<bbox_t> &detect_result);
