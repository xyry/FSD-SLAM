...
public:
    Segment(const string &pascal_prototxt, const string &detection_config, const string &pascal_caffemodel, const string &detection_weights, const string &pascal_png,float DetectionRatio);
    ...
    // Paremeters for caffe and detection
    string segment_model_file;
    string segment_prototxt_file;
    string segment_LUT_file;
    
    string detection_config_file;
    string detection_weights_file;
    
    
    Detector* detector;
    std::vector<bbox_t> mDetect_result_latest;
    std::vector<bbox_t> mDetect_result;
    std::vector<bbox_t> mDetect_result_temp;
    
    //the flag to show segment(1) or detection(0),
    int mDetect_segment_flag;
    
    //the threshold of ratio of detection area to segment, if higher, switch to segment 
    double mDetectionRatio;

    int num_segmentation;
    int num_object_detection;

    double time_segmentation;
    double time_object_detection;
