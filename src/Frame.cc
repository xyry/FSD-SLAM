...
void Frame::CalculEverything_segment( cv::Mat &imRGB, const cv::Mat &imGray,const cv::Mat &imDepth,const cv::Mat &imS)
{ 
    ...
    if(!T_M.empty() && flagprocess )
    {
        ...
        flag_mov = mpORBextractorLeft->CheckMovingKeyPoints_segment(imGray,imS,mvKeysTemp,T_M);
...
void Frame::CalculEverything_detection( cv::Mat &imRGB, const cv::Mat &imGray, const cv::Mat &imDepth, const std::vector<bbox_t> &detect_result )
{ 
    mDetect_result = detect_result;
    int flagprocess = 0;
    ...
    {
        std::chrono::steady_clock::time_point tc1 = std::chrono::steady_clock::now();
        flag_mov = mpORBextractorLeft->CheckMovingKeyPoints_detection(imGray,detect_result,mvKeysTemp,T_M);
        std::chrono::steady_clock::time_point tc2 = std::chrono::steady_clock::now();
        double tc= std::chrono::duration_cast<std::chrono::duration<double> >(tc2 - tc1).count();
        cout << "check time =" << tc*1000 <<  endl;
    }
