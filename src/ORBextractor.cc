...
int ORBextractor::CheckMovingKeyPoints_segment( const cv::Mat &imGray, const cv::Mat &imS,std::vector<std::vector<cv::KeyPoint>>& mvKeysT,std::vector<cv::Point2f> T)
{
...
bool ORBextractor::isInPerson( const cv::Point2f &coordinate,  const std::vector<bbox_t> &detect_result)
{
  bool InPerson = false;
  for(auto temp: detect_result)
  {
    if(temp.obj_id == 0)
    {
      if(coordinate.x>temp.x&&coordinate.y>temp.y&&coordinate.x<(temp.x+temp.w)&&coordinate.y<(temp.y+temp.h))
      {
	InPerson = true;
	break;
      }
...
int ORBextractor::CheckMovingKeyPoints_detection(const cv::Mat &imGray, const std::vector<bbox_t> &detect_result, std::vector<std::vector<cv::KeyPoint>>& mvKeysT, std::vector<cv::Point2f> T)
{
	    ...
	    for(int m = -15; m < 15; m++) 
	    {
		    ...
		    cv::Point2f my_coordinate(mx,my);
                // The id of peopel is 0
                    if(isInPerson(my_coordinate, detect_result))
		    {
		      flag_orb_mov = 1;
		      break;
		    }
	    ...
	    for (int level = 0; level < nlevels; ++level)
            {
                ...
                while(keypoint != mkeypoints.end())
	            {
		             ...
		             if(search_coord.y >= (Camera::height -1)) search_coord.y=(Camera::height -1) ;
		             //int label_coord =(int)imS.ptr<uchar>((int)search_coord.y)[(int)search_coord.x];
		             if(isInPerson(search_coord, detect_result)) 
		             {
			            keypoint=mkeypoints.erase(keypoint);		       
		             }
		             else
