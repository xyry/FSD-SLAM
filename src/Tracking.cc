...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, Map *pMap, boost::shared_ptr<PointCloudMapping> pPointCloud, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor,FrameDrawer* pFrameDrawer, MapDrawer* pMapDrawer):
   mSensor(sensor), mbOnlyTracking(false), mbVO(false), mbNewSegImgFlag(false),mpORBVocabulary(pVoc), mpPointCloudMapping( pPointCloud ),mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys),mpMap(pMap), mnLastRelocFrameId(0), mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer)
{
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    if(mpSegment->mDetect_segment_flag == 1)
    {
      mCurrentFrame.CalculEverything_segment(mImRGB,mImGray,mImDepth,mpSegment->mImgSegmentLatest);
    }
    else
    {
      mCurrentFrame.CalculEverything_detection(mImRGB,mImGray,mImDepth, mpSegment->mDetect_result_latest);
    }
    
    std::vector<bbox_t> mDetect_Result = mpSegment->mDetect_result_latest;
