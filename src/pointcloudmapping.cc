...
void PointCloudMapping::insertKeyFrame(KeyFrame* kf, cv::Mat& semantic_color,cv::Mat& semantic,cv::Mat& color, cv::Mat& depth)
{
    ...
    //cv::Mat self_semantic = cv::imread("/home/greenhand/PointCloudMapping/reseg_color/"+to_string(kf->mTimeStamp)+".png",CV_LOAD_IMAGE_UNCHANGED);
    //cout<<"//////////////////////keyframe:"<< to_string(kf->mTimeStamp)<<"////////////////////" << endl;
    //semanticImgs_color.push_back(self_semantic);
    ...
    for ( int m=0; m<depth.rows; m+=1)
    {
        for ( int n=0; n<depth.cols; n+=1)
        {
            ...
            if (d < 0.01 || d > 8)
	            ...
	            // Deal with color
	            if(semantic_color.ptr<uchar>(m)[n*3]==0&&semantic_color.ptr<uchar>(m)[n*3+1]==0&&semantic_color.ptr<uchar>(m)[n*3+2]==0)
	            {
                    /*p.b = 255;
                    p.g = 255;
                    p.r = 255;*/
